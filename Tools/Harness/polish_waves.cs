...
tick = () => {
    ...
    for (int i = 0; i < cells.Count; i++) {
        ...
        if (stage == 0 && i == 0 && height > .03f) near = true;
        if (stage == 0 && i == cells.Count - 1 && height > .03f) far = true;
    }
    ...
    if (stage == 0) check(near && far, "Corner ripple reaches source and opposite corner");
    stage++;
    if(stage == 5) {
