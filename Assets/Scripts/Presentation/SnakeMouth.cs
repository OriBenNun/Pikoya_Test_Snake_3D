...
namespace GardenSnake.Presentation
{
    ...
    public sealed class SnakeMouth : MonoBehaviour
    {
        ...
        public void Animate(GameLoopManager loop, Vector3 foodPosition, float delta)
        {
            if (delta <= 0) return;
            excitementAge += delta;
            var swallowing = swallowAge < swallowDuration;
            if (swallowing)
            {
                swallowAge += delta;
                var t = Mathf.Clamp01(swallowAge / swallowDuration);
                var destination = transform.TransformPoint(settings.SwallowDestination);
            ...
            var offset = foodPosition - transform.position;
            offset.y = 0;
            var ahead = Vector3.Dot(offset, transform.forward);
            var sideways = Mathf.Abs(Vector3.Dot(offset, transform.right));
            var target = loop.State == RunState.Playing && loop.HasFood && ahead > 0 && sideways < settings.AnticipationHalfWidth
        ...
        private void CreateEyeRigs(MeshRenderer[] parts)
        {
            foreach (var white in parts)
            {
                ...
                var rest = transform.InverseTransformPoint(white.transform.position);
                ...
                foreach (var part in parts)
                {
                    ...
                    var side = transform.InverseTransformPoint(part.transform.position).x;
        ...
        private void Pose()
        {
            ...
            var opening = Quaternion.Euler(-Mathf.Atan2(settings.UpperFaceLift, faceLever)
                                           * Mathf.Rad2Deg * Openness, 0, 0);
            for (var i = 0; i < upperFace.Count; i++)
            {
                var face = upperFace[i];
            ...
            {
                var eye = eyes[i];
                var rest = eye.RestPosition;
                var bounce = Mathf.Sin(excitementAge * settings.EyeBounceFrequency * Mathf.PI * 2 + i * .8f)
