...
`Tools/Blender/update_bush.py` updates only the bush in the source sheet and FBX. Full `create_assets.py` rebuilds retain the blossoms through `polish_assets.py`. Wildlife meshes are reproducibly authored and baked by `GardenWildlifeMeshes`; they are Unity meshes, not Blender rigs. Animation remains procedural. Browser build and target-device performance were not tested in this pass.
