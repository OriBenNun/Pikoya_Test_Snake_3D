...
shake, freeze frame, screen flash, slow motion and UI spring. `SnakeFeel` is the only seam between
the simulation and the noise it makes, so the game reports *what* happened and never *how loud*.
...
- `Assets/GardenSnake/Scripts/SnakeController.cs`: input, tick timing, presentation, audio, camera.
- `Assets/GardenSnake/Scripts/SnakeFeel.cs`: the Feel players the controller triggers.
- `Assets/GardenSnake/Scripts/SnakeHud.cs`: HUD, start/pause/results flow, and UI feedback.
- `Assets/GardenSnake/Scripts/GardenWind.cs`: one clock that sways the whole meadow.
- `Assets/GardenSnake/Editor/GardenBuilder.cs`: repeatable material, prefab, scene and build authoring.
- `Assets/GardenSnake/Editor/GardenPalette.cs`: every colour in the game, in one place.
- `Assets/GardenSnake/Editor/GardenHud.cs`, `GardenSprites.cs`: the interface and the shapes it is
  drawn from, generated straight into PNG assets so no UI art is imported.
- `Assets/GardenSnake/Editor/GardenFeel.cs`: authors the MMFeedbacks players.
- `Assets/GardenSnake/Editor/GardenPlaythrough.cs`: a Play mode driver that uses real input.
- `Assets/GardenSnake/Editor/GardenPlaytest.cs`: an unattended autopilot that plays the game and
...
to preserve across rebuilds. Ordinary tuning can be done on the `Snake Game` component in the
Inspector.
